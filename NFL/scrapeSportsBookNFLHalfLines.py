...
db = sqlite3.connect('/home/ec2-user/sports2015/NFL/sports.db')

x=random.randint(3,10)
time.sleep(x)

#url = urllib2.urlopen('https://www.sportsbook.ag/sbk/sportsbook4/nba-betting/nba-2nd-half-lines.sbk')
url = urllib2.urlopen('https://www.sportsbook.ag/sbk/sportsbook4/nfl-betting/2nd-half-lines.sbk')
...
for i in range(0, len(lines)):
    try:
        with db:
            db.execute('''INSERT INTO NFLSBHalfLines(away_team, home_team, line, spread, game_date, game_time) VALUES(?,?,?,?,?,?)''', (awayTeams[i], homeTeams[i], lines[i], spreads[i], today, date_time))
            db.commit()
...
for i in range(0, len(lines)):
    try:
        with db:
            db.execute('''INSERT INTO NFLSBteamlookup(sb_team, espn_abbr) VALUES (?,?)''', (awayTeams[i], None))
            db.execute('''INSERT INTO NFLSBteamlookup(sb_team, espn_abbr) VALUES (?,?)''', (homeTeams[i], None))
    except:
        print 'Record Exists'
