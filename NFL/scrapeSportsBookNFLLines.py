...
url = urllib2.urlopen('https://www.sportsbook.ag/sbk/sportsbook4/nfl-betting/nfl-lines.sbk')
...
for i in range(0, len(lines)):
    try:
        with db:
            db.execute('''INSERT INTO NFLSBLines(away_team, home_team, line, spread, game_date, game_time) VALUES(?,?,?,?,?,?)''', (awayTeams[i], homeTeams[i], lines[i], spreads[i], today, date_time))
            db.commit()
